...
<%
    ...
    try {
        ...
        // 예외처리
        if (!idValue.matches("^(?=.*[A-Za-z])(?=.*\\d)[A-Za-z\\d]{8,12}$")) {
        ...
        // sql 준비 및 전송
        String sql = "SELECT * FROM user WHERE id=? AND password=?"; // 입력된 아이디와 비밀번호를 가진 사용자 조회
        PreparedStatement query = conn.prepareStatement(sql);
        query.setString(1, idValue); // 첫 번째 Column 읽어오기
        query.setString(2, pwValue); // 두 번째 Column 읽어오기

        ResultSet result = query.executeQuery();  // sql 결과 받아오기
        ...
        if (!result.next()) {   // MEMO : 조건문으로는 실패의 경우를 먼저 써주는게 좋음
            ...
            // 로그인 성공 시에 필요한 처리
            HttpSession userSession = request.getSession(); // 세션에 사용자 정보 저장
            userSession.setAttribute("id", idValue); // "id"는 세션에 저장될 속성의 이름, idValue는 그에 해당하는 값

            out.println("<script>location.href='../page/main.jsp';</script>"); // 로그인 성공 시에 main.jsp로 이동
        }
    } catch (Exception e) {
